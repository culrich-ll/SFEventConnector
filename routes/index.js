...
router.get('/create-case', function (req, res, next) {
  res.render('create-case', { title: 'Create Case' });
});
...
router.get('/cases', function (req, res, next) {
  ...
  org.query({ query: "Select Id, Serial_number__c ,External_ID__c, Subject, Origin From Case Order By LastModifiedDate DESC" })
    .then(function (results) {
      res.render('index-cases', { records: results.records });
    });
...
router.post('/delete-case', function (req, res, next) {

  var cs = nforce.createSObject('Case');
  cs.set('Id', req.body.id);
  console.log('id: %s', req.body.id);
  org.delete({ sobject: cs })
    .then(function (msg) {
      res.render('message', { title: 'Case Deleted: ' + String(req.body.id) });
    });
});
